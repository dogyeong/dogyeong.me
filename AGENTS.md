...
- **스타일**: 전역 SCSS 변수 파일(`assets/styles.scss`, 프로젝트 루트)이 `astro.config.mjs`의 Vite `css.preprocessorOptions.scss.additionalData` 설정으로 모든 `<style lang="scss">`에 자동 주입된다. 따라서 import 없이 `$grey-0~9`, `$indigo-0~9` 와 브레이크포인트를 바로 쓴다. 미디어 쿼리는 `@media ($tablet)`, `@media ($desktop)` 형태(변수 안에 `min-width: ...` 문자열이 들어있다). 컴포넌트 스타일은 `.astro` 파일의 스코프드 `<style>` 블록을 쓴다(Astro가 클래스명을 자동으로 스코프하므로 Vue의 CSS Modules 같은 별도 문법이 필요 없다). Sass 모듈(`sass:color` 등)이 필요하면 파일 안에 `@use`를 평소처럼 적으면 된다 — 주입된 `@use`가 앞에 붙어 두 번째 `@use`가 되는데, `@use`끼리는 연속해도 되므로 문제없다. 전역 내장 함수(`darken()` 등)는 Dart Sass 3.0에서 제거되므로 `color.adjust()` 같은 모듈 함수를 쓴다.
...
- **`markdown.smartypants: false`는 지우면 안 된다.** Astro는 이 값을 기본으로 켜서 곧은 따옴표·아포스트로피·`...`·`--`를 굽은 따옴표와 기호로 바꾼다. Nuxt Content는 이 변환을 하지 않았으므로, 끄지 않으면 **이미 발행된 글의 본문 글자가 바뀐다**(마이그레이션 당시 47편 중 15편에서 실측). 빌드 산출물 비교 없이는 lint도 빌드도 아무 신호를 주지 않는다.
- **빌드마다 Astro deprecation 경고 2개가 뜬다** — `markdown.rehypePlugins`와 `markdown.smartypants`를 `unified()`/`satteri()`로 옮기라는 내용이다. 정상이며, 안내대로 고치려면 위 두 가지(헤딩 앵커, 따옴표)의 동작을 그대로 재현하는지 산출물로 확인해야 한다. `astro`가 `7.1.3`으로 고정돼 있어 당장 깨지지 않으니, 메이저를 올릴 때 함께 다룬다.
